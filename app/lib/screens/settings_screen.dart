    ...
    return _Section(
      ...
      children: [
        ...
        _DropdownRow(
          ...
          (v) => _set(context, (x) => x.remoteDns = v ?? x.remoteDns,
              reload: true),
        ),
      ],
    );
      ...
      children: [
        _DropdownRow(
          ...
          (v) => _set(
            context,
            (x) =>
                x.routeMode = RouteMode.values.firstWhere((e) => e.name == v),
            reload: true,
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return _Section(
      ...
      children: [
        _SwitchRow(
            '自动故障转移',
            '探测失败后切换备用节点（Passwall 风格）',
            s.autoFailover,
            (v) => context
                .read<SettingsProvider>()
                .set((x) => x.autoFailover = v)),
        _SwitchRow(
            '恢复主节点',
            '主节点恢复后自动切回',
            s.restorePrimary,
      ...
      children: [
        _DropdownRow(
          ...
          (v) => context.read<SettingsProvider>().set(
                (x) => x.themeMode =
                    ThemeMode.values.firstWhere((e) => e.name == v),
              ),
        ),
      ],
    );
  ...
  Widget build(BuildContext context) {
    ...
    switch (updates.status) {
      case UpdateStatus.available:
        statusText = '发现 v${updates.latestVersion}';
        statusColor = NexusColors.warn;
        break;
      case UpdateStatus.upToDate:
        statusText = '已是最新版本';
        statusColor = NexusColors.ok;
        break;
      case UpdateStatus.error:
        statusText = updates.errorMessage ?? '自动检查暂不可用';
        statusColor = NexusColors.danger;
        break;
      case UpdateStatus.checking:
        statusText = '正在检查 GitHub Releases…';
        break;
      case UpdateStatus.idle:
        statusText = '当前 v${updates.currentVersion}';
        break;
    }
    ...
    return _Section(
      ...
      children: [
        _SwitchRow(
          '自动检查',
          '应用与 sing-box 随 Release 一起更新',
          settings.autoUpdate,
          (v) {
            context.read<SettingsProvider>().set((x) => x.autoUpdate = v);
            if (v) updates.configureAutomaticCheck(true);
          },
        ),
        Padding(
          ...
          child: Row(
            children: [
              ...
              if (updates.hasUpdate)
                ...
                OutlinedButton(
                  onPressed: checking ? null : () => updates.checkForUpdates(),
...
class _DropdownRow extends StatelessWidget {
  ...
  const _DropdownRow(
      this.name, this.value, this.items, this.labels, this.onChanged);
